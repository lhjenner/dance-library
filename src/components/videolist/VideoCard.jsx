export default function VideoCard({ 
  ...
  return (
    <div className="bg-gray-800 rounded-lg p-4 flex gap-4 hover:bg-gray-750 transition-colors">
      {isSelectionMode && (
        <input
          type="checkbox"
          checked={isSelected}
          onChange={() => onToggleSelection(video.id)}
          className="w-5 h-5 rounded border-gray-600 bg-gray-700 text-blue-600 focus:ring-blue-500 focus:ring-offset-gray-900 cursor-pointer flex-shrink-0 mt-1"
        />
      )}
      {video.thumbnail && (
        <img
